...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            output = [to_list(outputs[output][i]) for output in outputs]
    ...
    logger.info(
        "***** Transform the prediction file into the BioASQ format {} *****".format(prefix))
    ...
    eval_score = eval_bioasq_standard(
        str(5), pred_file, args.golden_file, args.official_eval_dir)
